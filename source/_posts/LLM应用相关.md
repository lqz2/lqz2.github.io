...
- 单向性：SSE 只能服务器推给客户端。这导致 MCP 必须用两个连接：一个 HTTP POST 用来发请求，一个 SSE 用来收回复。
- 代理不友好：很多现代的反向代理（如某些云端负载均衡）对保持长期的 SSE 连接支持不佳。
- 开销：SSE 的 data: ...\n\n 格式在传输高频、复杂的 JSON-RPC 消息时显得有些啰嗦。

### Streamable HTTP的优势
- 全双工：不再需要分两个通道，请求和响应可以在同一个 HTTP 连接流中交替进行。
- 原始数据传输：它不再强制要求 text/event-stream 格式，而是直接传输 JSON 块，减少了协议头的解析负担。
- 更好的基础设施兼容性：它更符合现代 gRPC 或 REST 流的标准，能够更容易穿透复杂的企业防火墙和 CDN。

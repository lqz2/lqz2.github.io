...
### 分布式事务有哪些解决方案？
- 2PC，两阶段提交协议，分为准备阶段和提交阶段，准备阶段由协调者通知参与者执行事务但不提交，提交阶段由协调者统计所有参与者的执行状态，如果都成功则通知所有参与者提交，否则通知所有参与者回滚，常见的2PC有Seata的XA和AT模式
- 3PC, 引入了预提交阶段，引入超时机制，性能仍然不高，整体执行时间长
- TCC，Try-Confirm-Cancel模式，每一步都需要代码手动实现，补偿事务
- 本地事务表+消息队列，将分布式事务拆分为本地事务进行处理，通过消息中间件保证最终一致性。具体来说，每个本地事务执行成功后，可以记录在本地消息表中，然后由定时任务定期扫描待发送的消息，发送给消息队列，消费者再执行相应的业务逻辑
- SAGA, 补偿事务，将长事务拆分为多个短事务（本地事务）。依次执行各参与者的正向操作。如果所有正向操作均成功，事务提交。如果任何一个正向操作失败，会以相反的顺序执行前面已提交参与者的逆向回滚操作（补偿操作），使分布式事务回到初始状态。

...
- PriorityBlockingQueue，支持优先级的无界阻塞队列。队列中的元素会根据其自然顺序或在构造时提供的 Comparator 进行排序，优先级高的元素会先被执行

| 特性 | SynchronousQueue (直接提交队列) | LinkedBlockingQueue (链表阻塞队列) | ArrayBlockingQueue (数组阻塞队列) | PriorityBlockingQueue (优先级队列) |
| :--- | :--- | :--- | :--- | :--- |
| **数据结构** | - | 链表 | 数组 | 堆 (Heap) |
| **容量** | **0 (零容量)**，不存储任何元素。 | **无界 (默认)** 或 **有界**。默认容量为 `Integer.MAX_VALUE`。 | **有界**，必须在创建时指定容量。 | **无界**，容量理论上为 `Integer.MAX_VALUE`。 |
| **排序方式** | 直接提交，无排序。 | **FIFO** (先进先出)。 | **FIFO** (先进先出)。 | **按优先级**，由任务的自然顺序或 `Comparator` 决定。 |
| **工作机制** | 任务直接交给空闲线程。若无空闲线程，则尝试创建新线程（直到`maximumPoolSize`），否则任务被拒绝。 | **[无界时]** 核心线程满后，任务会无限加入队列，`maximumPoolSize` 参数失效。 | 核心线程满后，任务进入队列。队列满后，创建新线程（直到`maximumPoolSize`），否则任务被拒绝。 | 核心线程满后，任务按优先级加入队列，`maximumPoolSize` 参数失效。 |
| **适用场景** | 吞吐量高，处理大量**短时**任务的场景。要求系统响应快，能快速处理任务。 | 任务量不确定，不希望任务被拒绝的场景。适用于**任务突发**的情况。 | 需要精确控制资源（线程数、任务数），防止系统过载的场景。 | 任务具有不同优先级，需要优先执行**重要任务**的场景。 |
| **风险/注意事项** | `maximumPoolSize` 需设置得很大（如 `Integer.MAX_VALUE`），否则任务极易被拒绝。 | **[无界时]** 任务可能大量堆积，导致**内存溢出 (OOM)**。 | 队列容量和 `maximumPoolSize` 的设置需仔细权衡，不合理会降低吞吐量或频繁拒绝任务。 | 同样存在**内存溢出 (OOM)** 的风险。任务优先级设置不当可能导致低优先级任务**饥饿**。 |
...
### 二级索引（非聚簇索引）有MVCC快照吗？
MVCC主要包含隐藏列、undolog、readview三部分，二级索引是没有隐藏列的，也就是没有最近提交的事务id和回滚指针。
- 在覆盖索引的情况下，如果索引的数据没有被修改，那么二级索引是可以利用覆盖索引返回数据的，无需回表。
- 如果索引的数据被修改了，那么需要回表，去聚簇索引中查找数据行。
...
### 深度分页有什么优化方法？
- 使用范围查询：通过上一页最后一条记录的ID作为起始ID，但这种方式如果遇到不连续非自增的ID就不适用了
- 使用子查询：在子查询中通过主键索引快速定位到相应的数据ID，然后在主查询中，使用子查询的结果作为过滤条件，但子查询会生成临时表，在复杂场景下性能会下降
- 使用延迟关联：和子查询思想类似，不同的是通过内连接(inner join)将子查询的结果集成到主查询中，避免了产生临时表，所以性能会优于子查询
- 使用覆盖索引：通过覆盖索引来避免回表查询，提升性能，但是如果查询结果集很大（超过总数据的30%），mysql的查询优化器可能会选择全表扫描而不是使用索引
...
- 在查询中对索引列使用函数时
- 联合索引没有遵循最左匹配原则
- 范围查询`< 和 >`右边的字段，索引会失效，但是`>=, <=, between, like前缀匹配`右边的字段不会
- 查询字段发生类型转换
...
### LRU和LFU的缺点？
- LRU主要淘汰最久未使用的数据，如果某些数据比较重要，而且在上一段时间内使用很频繁，如果有新数据到来，也容易被淘汰
- LFU主要淘汰使用频率最低的数据，它需要维护逻辑访问次数，会增加一些计算开销，同时对于新来的数据，由于访问频率较低，可能很快就被淘汰
...
- 主从模式下，某个客户端获得锁之后，主节点宕机，此时主节点的锁数据还没来得及同步到从节点，选出新的主节点后，另一个客户端可能会成功获取同一把锁，违背了分布式锁的互斥性
...
### 怎么保证redis里都是热点数据？
- 冷启动预热，根据历史数据，将热点数据提前加载到缓存中
- 统计热点数据，及时发现热点
- redis内存淘汰策略，lru或lfu淘汰不常用的key
- 使用本地缓存
...
## AOP
### 什么是AOP？
AOP是将一些横切关注点如日志、事务、权限控制等与核心业务逻辑分离，从而提高应用程序的模块化程度，通过动态代理等技术实现代码的复用和解耦，提高代码的可维护性和可扩展性。

### AOP的一些术语有哪些？
- 横切关注点：多个类或者对象的公共行为，如日志、事务、权限控制等模块。
- 切面：对横切关注点进行封装的类，一个切面是一个类
- 连接点：特定时刻某个方法的执行
- 通知：在某个连接点要执行的操作
- 切点：一组连接点的集合
- 织入：将切面和目标对象连接起来的过程，Spring AOP在运行时执行织入
...
### Spring中事务的传播行为有哪些？
支持当前事务的：
- required：如果当前没有事务，则新建事务，如果当前存在事务，则加入当前事务，默认传播级别
- supports：如果当前没有事务，则以非事务方式执行，如果当前存在事务，则加入当前事务
- mandatory：如果当前没有事务，则抛出异常，如果当前存在事务，则加入当前事务

不支持当前事务的：
- requires_new：新建事务，如果当前存在事务，则挂起当前事务
- not_supported：以非事务方式执行，如果当前存在事务，则挂起当前事务
- never：以非事务方式执行，如果当前存在事务，则抛出异常
- nested：如果当前没有事务，则新建事务，如果当前存在事务，则在嵌套事务中执行

### 事务失效的场景有哪些？
- 出现了未捕获的异常或者非受检异常比如RuntimeException (非受检异常是指在代码中可能出现的异常，但不需要显式地进行捕获或声明，比如空指针或者数组越界等)
- 事务的传播行为设置的不正确
- 多数据源情况下，事务管理没有正确配置
- 同一个类中方法内部调用
- 非public方法使用@Transactional注解会失效

### 为什么方法内部调用@Transactional注解会失效？
答：@Transactional注解是基于AOP实现的，AOP又是通过动态代理来实现的，因为同一个类中方法内部调用时，是通过this调用的，this调用不会经过代理对象，所以代理对象的逻辑不会生效
...
### 为什么是三级缓存而不是二级缓存？
三级缓存中的ObjectFactory可以用来创建代理对象，他的核心作用是**将Bean的实例化和Bean的代理对象创建这两个操作解耦**。它推迟了代理对象的创建时机，只有在真正发生循环依赖并需要注入时，才通过工厂创建代理对象。

如果仅使用二级缓存，就无法解决AOP的循环依赖问题，因为二级缓存只能存放半成品的Bean实例，无法存放创建代理对象的逻辑。除非将AOP代理的创建时机提前，这样违背了spring的设计原则。
...
### 分布式事务有哪些解决方案？
- 2PC，两阶段提交协议，分为准备阶段和提交阶段，准备阶段由协调者通知参与者执行事务但不提交，提交阶段由协调者统计所有参与者的执行状态，如果都成功则通知所有参与者提交，否则通知所有参与者回滚，常见的2PC有Seata的XA和AT模式。缺点有阻塞问题、单点故障、性能较低等
- 3PC, 引入了预提交阶段，引入超时机制，性能仍然不高，整体执行时间长。
- TCC，Try-Confirm-Cancel模式，每一步都需要代码手动实现，补偿事务。缺点是有代码侵入，实现复杂，要处理空回滚、防悬挂和幂等性问题。
- 本地事务表+消息队列，将分布式事务拆分为本地事务进行处理，通过消息中间件保证最终一致性。具体来说，每个本地事务执行成功后，可以记录在本地消息表中，然后由定时任务定期扫描待发送的消息，发送给消息队列，消费者再执行相应的业务逻辑。缺点是最终一致性而非强一致。
- SAGA, 补偿事务，将长事务拆分为多个短事务（本地事务）。依次执行各参与者的正向操作。如果所有正向操作均成功，事务提交。如果任何一个正向操作失败，会以相反的顺序执行前面已提交参与者的逆向回滚操作（补偿操作），使分布式事务回到初始状态。缺点和TCC一样，要处理空回滚、防悬挂等问题。
...
### 怎么选择分布式事务方案？
- 2PC和3PC：适用于强一致性要求的场景，如金融、支付场景
- 吞吐量高，最终一致性场景：本地事务表+消息队列
- TCC、SAGA：业务复杂，需提供灵活的补偿机制的场景

### 空回滚和防悬挂问题怎么解决？
- 空回滚指的是即使try没有成功，也执行cancel操作。解决方法是，在try阶段后将事务ID和预留的资源或状态绑定，在cancel阶段检查预留的资源或状态，如果没有预留资源或状态，则cancel直接返回，不执行回滚逻辑

- 防悬挂是cancel比try先执行，原因一般是网络延迟或并发导致的。解决方法也是维护一个事务状态表，在try之前检查是否执行过cancel操作，如果已经执行过cancel操作，则try直接返回失败。
...
## 负载均衡
### 负载均衡怎么分类？
- 服务端负载均衡：应用在系统外部请求和网关层之间，最常见的是四层负载均衡和七层负载均衡
- 客户端负载均衡：应用于系统内部的不同服务
...
- 四层负载均衡主要工作在OSI模型的第四层，也就是传输层，这一层主要是TCP和UDP协议，四层负载均衡主要是根据IP和端口进行负载均衡，不涉及具体的报文内容
- 七层负载均衡主要工作在OSI模型的第七层，也就是应用层，这一层主要是HTTP协议，七层负载均衡可以根据HTTP报文的内容进行负载均衡，执行七层负载均衡的设备被称为反向代理服务器
- 一般用LVS做四层负载均衡，用nginx做七层负载均衡

### 负载均衡算法有哪些？
- 随机，加权随机，两次随机
- 轮询，加权轮询，平滑的加权轮询
- 哈希，一致性哈希，普通哈希如果节点数量变化，会导致大部分节点哈希映射变化，一致性哈希将数据和节点都映射到一个环上，当增加或者删减节点的时候，请求会沿着环的顺时针方向找到第一个节点，也就是说，一致性哈希只会影响到相邻的后继节点，不会影响整个集群的哈希映射分布。
    - 但一致性哈希的问题是，如果节点分布不均匀，某个节点可能请求过多，导致负载不均衡，解决方法是增加虚拟节点，使分布更加均匀
- 最少连接数、最少活跃连接数、最快响应等


### 七层负载均衡可以怎么做？
- DNS解析：用户请求域名，DNS服务器解析成IP地址，返回给用户
- Nginx反向代理：用户将请求发送到nginx，nginx根据负载均衡算法将请求转发到后端服务器

### 正向代理和反向代理的区别？
- 正向代理代理的是客户端，帮助客户端访问其无法直接访问的服务器资源；
- 反向代理代理的是服务器端，为服务器收发请求，客户端感知不到反向代理后面的真实服务器。
...
- TCP是面向连接的，UDP是无连接的
- TCP是可靠的，提供流量控制、拥塞控制、数据校验等机制，UDP是不可靠的，只是尽最大努力传递数据
- TCP是面向字节流的，UDP是面向报文的
- TCP的传输速度相对较慢，UDP传输速度快
- TCP适用于对数据传输要求高的场景，如文件传输、电子邮件等，UDP适用于对实时性要求高的场景，如视频会议、在线游戏
